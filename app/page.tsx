...
export default function AayanAudioTrainingApp() {
  ...
  useEffect(() => {
    if (bgmRef.current) {
      ...
      if (!isMuted) {
        bgmRef.current.play().catch(() => console.log("タップ待ち..."));
      } else {
  ...
  const handleSelectCat = (cat: typeof CATS[0]) => {
    setSelectedCat(cat);
    setIsMuted(false); // 猫を選んだ瞬間に音をONにする試み
  ...
  if (!selectedCat) {
    return (
      <div style={{ width: '100vw', height: '100vh', background: '#ff9ebb', fontFamily: '"Zen Maru Gothic", sans-serif', padding: '20px', textAlign: 'center', overflowY: 'auto' }}>
        ...
        <h1 style={{ color: 'white', textShadow: '2px 2px 0 #d46' }}>新しく始めるパートナーを選んで！</h1>
    ...
    <main style={{ width: '100vw', height: '100vh', fontFamily: '"Zen Maru Gothic", sans-serif', backgroundImage: 'url("/bg-track.png")', backgroundSize: 'cover', backgroundPosition: 'center', paddingBottom: '300px' }}>
      ...
      <div style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.85)', borderBottom: '4px solid #ff4d8d', textAlign: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px' }}>
          ...
          <button onClick={() => setIsMuted(!isMuted)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer' }}>
            {isMuted ? '🔇' : '🔊'}
          </button>
        </div>
      ...
      <div style={{ padding: '20px', textAlign: 'center' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.7)', padding: '10px', borderRadius: '15px', color: 'white', fontSize: '12px', marginBottom: '10px' }}>
          ...
          <div style={{ width: '100%', height: '8px', background: '#333', borderRadius: '4px', marginTop: '4px', overflow: 'hidden' }}>
            <div style={{ width: `${hp}%`, height: '100%', background: '#4caf50' }} />
          </div>
        </div>

        <div style={{ height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: '160px', height: '160px', backgroundImage: 'url("/cats-lineup.png")', backgroundSize: '500% 200%', backgroundPosition: selectedCat.pos }} />
        </div>

        {/* 履歴 */}
        <div style={{ background: 'rgba(255,255,255,0.9)', borderRadius: '15px', padding: '10px', maxHeight: '120px', overflowY: 'auto' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, width: '100%', background: 'white', padding: '15px', borderTop: '5px solid #ff4d8d', borderRadius: '25px 25px 0 0' }}>
        ...
        <button onClick={handleSave} style={{ width: '100%', padding: '15px', background: '#ff4d8d', color: 'white', borderRadius: '30px', fontWeight: 'bold' }}>特訓をきろく！</button>
      </div>
    </main>
  );
}
